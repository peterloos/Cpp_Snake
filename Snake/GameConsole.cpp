...
//         DWORD dwEventsRead;
//
//         // read data from the specified console input buffer
//         // without removing it from the buffer
//         ::PeekConsoleInput (m_hStdin, &evt, 1, &dwEventsRead);  
//
//         if (evt.EventType == KEY_EVENT && evt.Event.KeyEvent.bKeyDown == TRUE)
//         {
//             ::ReadConsoleInput (m_hStdin, &evt, 1, &dwEventsRead);
//             m_lastVirtualKey = evt.Event.KeyEvent.wVirtualKeyCode;   
//             ::FlushConsoleInputBuffer(m_hStdin);
//             return true;  
//         }   
//         else  
...
void GameConsole::ReadInput()
{
    ...
    {
        case VK_UP:
            if (m_lastValidArrow != Down)
                m_lastValidArrow = Up;
            break;
        case VK_DOWN:
            if (m_lastValidArrow != Up)
                m_lastValidArrow = Down;
            break;
        case VK_RIGHT:
            if (m_lastValidArrow != Left)
                m_lastValidArrow = Right;
            break;
        case VK_LEFT:
            if (m_lastValidArrow != Right)
                m_lastValidArrow = Left;
            break;
        case VK_ESCAPE:
            m_isEscapeHit = true;
            break;
